...
class SigninTest(TestCase):
    ...
    def test_login(self):
        # send login data
        response = self.client.post(reverse('signin:signin'), self.credentials, follow=True)
    ...
    def test_wrong_username(self):
        ...
        response = self.client.post(reverse('signin:signin'), self.wrongUsername, follow=True)
        self.assertFalse(response.context['user'] is not None and response.context['user'].is_authenticated)
        #set_trace()
    ...
    def test_wrong_password(self):
        ...
        response = self.client.post(reverse('signin:signin'), self.wrongPassword, follow=True)
        self.assertFalse(response.context['user'] is not None and response.context['user'].is_authenticated)
...
class PasswordResetTest(TestCase):
    ...
    def test_b_send_password_reset_email(self):
        #set_trace()
        response = self.client.post(reverse('signin:password_reset'), {'email': self.credentials['email']})
    ...
    def test_c_password_reset(self):
        ...
        response1 = self.client.get(reverse('signin:password_reset_confirm', kwargs={'token':token,'uidb64':uid}))
        self.assertEqual(response1.status_code, 302)
        # Get the redirect URL
        url_response1=response1.url
        #set_trace()
        # Now we post to the url with our new password:
        response3 = self.client.post(url_response1, {'new_password1':'Fallnyucs@2019','new_password2':'Fallnyucs@2019'})
        ...
        self.assertEqual(response4.template_name[0], 'signin/password_reset_complete.html')


    def test_d_get_password_reset_complete_page(self):
        response=self.client.get(reverse('signin:password_reset_complete'))
        self.assertEqual(response.template_name[0], 'signin/password_reset_complete.html')
