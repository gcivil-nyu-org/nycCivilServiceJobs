...
civil_service_titles = (
    job_record.objects.all().values_list("civil_service_title").distinct()
)
...
class JobFilter(FilterSet):
    ...
    civil_service_title = ChoiceFilter(
        choices=getcivilservicetitle(),
        method="custom_search",
        label="Civil Service Title",
        empty_label="Civil Service Title",
    )
    ...
    business_title = ChoiceFilter(
        choices=getbusinesstitle(),
        method="custom_search",
        label="Business Title",
        empty_label="Business Title",
    )
    agency = ChoiceFilter(
        choices=getacgencies(),
        method="custom_search",
        label="Agency",
        empty_label="Agency",
    )
    ...
    def custom_search(self, queryset, name, query):
        ...
        if name == "civil_service_title":
            ...
            return queryset.filter(
                civil_service_title=civil_service_titles[int(query)][0]
            )
        if name == "agency":
